...
const calls = { write: [], uri: [], share: [], fallback: 0 };
let urlId = 0;
...
const URLMock = {
  createObjectURL() { return `blob:mock-${++urlId}`; },
  revokeObjectURL() {}
};
...
const context = {
  ...
  Capacitor: {
    ...
    Plugins: {
      Filesystem: {
        async writeFile(options) { calls.write.push(options); },
        async getUri(options) {
          calls.uri.push(options);
          return { uri: `file:///cache/${options.path}` };
        }
      },
      Share: {
        async share(options) { calls.share.push(options); return {}; }
      }
    }
  }
};
...
new vm.Script(source, { filename: 'native-compat.js' }).runInContext(context);

const payload = JSON.stringify({ hello: 'fabrication' });
const blob = new Blob([payload], { type: 'application/json' });
const href = context.URL.createObjectURL(blob);
const anchor = new context.HTMLAnchorElement();
anchor.href = href;
anchor.download = 'Job-TEST-Fabrication.json';
anchor.click();
...
const decoded = Buffer.from(calls.write[0].data, 'base64').toString('utf8');
if (decoded !== payload) throw new Error('Export JSON bytes changed in the native bridge.');
