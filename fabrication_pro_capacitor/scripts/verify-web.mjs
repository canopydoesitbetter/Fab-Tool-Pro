...
const requiredFiles = ['www/index.html','www/styles.css','www/ux.css','www/app.js','www/ux.js','www/calculator.js','www/native-compat.js'];
...
const scriptOrder = ['native-compat.js','app.js','ux.js','calculator.js'];
let previous = -1;
...
for (const stylesheet of ['styles.css','ux.css']) {
  if (!html.includes(`<link rel="stylesheet" href="${stylesheet}" />`)) throw new Error(`index.html must directly load ${stylesheet}.`);
}
...
for (const [name, source] of [['app.js',app],['ux.js',ux],['calculator.js',calculator],['native-compat.js',native]]) {
  new vm.Script(source,{filename:name});
  if (/createElement\(\s*['"]script['"]\s*\)/.test(source)) throw new Error(`${name} dynamically creates a shipped script loader.`);
}
const shippedJs=app+ux+calculator+native;
...
if (/\.tool-menu|\.tool-tab/.test(styles+uxStyles)) throw new Error('Legacy navigation CSS remains in canonical stylesheets.');
console.log('Canonical web source architecture: OK');
console.log('JavaScript syntax: OK (app.js, ux.js, calculator.js, native-compat.js)');
